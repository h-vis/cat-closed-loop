...
- 猫は魚まで移動してから取得し、元の場所に戻ります。ただし同じ切り分けで箱にも到達できる場合は、魚の手前の到着位置から直接箱へ向かいます。クリア時は、そのステージの猫が箱に入った絵を盤面とクリアカードに表示します。スタート地点にはクッションを置き、次の分割での判定位置を維持します。紙袋で移動済みの場合は、魚取得前の移動先へ戻ります。
...
猫版追加・イベント移動・描画解像度・操作・チュートリアルの37テストは成功。
