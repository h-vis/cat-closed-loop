...
const lessons = [
  ['cat-1', ['ひとふでで、お部屋を分けよう', '何もないマスをドラッグして、ひと続きの線を描きます。線と盤面の端で空間を区切りましょう。猫は直接動かさず、同じ空間にいるものがポイントです。線は枝分かれできません。'], ['One line, a cozy room', 'Drag across empty cells to draw one continuous line. Divide the room using the line and board edges. The cat stays put: what matters is which objects share its space. Your line cannot branch.']],
  ['fish', ['まずは、おさかな', '猫と魚が同じ空間に入ると、自動で魚をもらえます。魚を取ると、段ボールが開きます。'], ['A fish for your friend', 'Put the cat and fish in the same space to collect the fish automatically. The cardboard box will open.']],
  ['box-open', ['段ボールで、ひとやすみ', '魚を取ったあと、猫と段ボールを同じ空間に入れればクリア。クリア画面をタップすると次のお部屋へ進めます。'], ['A little place to rest', 'After collecting the fish, put the cat and box in the same space to clear the stage. Tap the cleared board to visit the next room.']],
